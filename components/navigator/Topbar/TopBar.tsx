...
const TopBar = ({ isOpen, setIsOpen }: any) => {
  ...
  return (
    ...
    >
      ...
      <View className="flex flex-row items-center justify-center" style={{columnGap:20}}>
        <TouchableOpacity onPress={()=>router.push("/(mine)/notification")}>
          <Icon size={30} iconURL={IconURL.notification}/>
        </TouchableOpacity>
        <TouchableOpacity className="" onPress={()=>setIsCreateGroupOpen(true)}>
              <Icon iconURL={IconURL.editable} size={30}/>
            </TouchableOpacity>
       <MyAvatar size={47}/>
       </View>
       {isCreateGroupOpen? <CreateGroupForm isVisible={isCreateGroupOpen} onClose={()=>setIsCreateGroupOpen(false)}/>: null}
    </View>
  );
};
