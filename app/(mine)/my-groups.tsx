...
import { getMyGroups } from "@/lib/message-request";
...
const MyGroupsPage = () => {
  ...
  return (
    <View className={`${bgLight500Dark10} flex-1`}>
      ...
      <ScrollView>
      {q==="" && groups? groups.map((item,index)=><MessageBox key={index} {...item}/>)
      :handleSearchGroups().map((item, index)=><MessageBox key={index} {...item}/>)}
      </ScrollView>
    </View>
  );
};
